...
- (NSString *)punycodeEncodedString {
	...
	for (j = 0;  j < input_length;  ++j) {
		if (basic(longchars[j])) {
			[ret appendFormat:@"%lu", longchars[j]];
			++outLen;
		}
	}
		...
		for (j = 0;  j < input_length;  ++j) {
			...
			if (c == n) {				
				for (q = delta, k = base;  ;  k += base) {
					...
					if (q < t)
						break;
					[ret appendFormat:@"%hhd", encode_digit(t + (q - t) % (base - t), 0)];
					q = (q - t) / (base - t);
				}
				
				[ret appendFormat:@"%hhd", encode_digit(q, 0)];
...
- (NSString *)decodedURLString {
  ...
  if (decodedPath == nil) {
    decodedPath = [path stringByReplacingPercentEscapesUsingEncoding:NSWindowsCP1251StringEncoding];
  }

	NSString *ret = [NSString stringWithFormat:@"%@%@%@%@", [urlParts objectForKey:@"scheme"], [urlParts objectForKey:@"delim"], [[urlParts objectForKey:@"host"] IDNADecodedString], decodedPath];
	if ([urlParts objectForKey:@"fragment"])
		ret = [ret stringByAppendingFormat:@"#%@", [urlParts objectForKey:@"fragment"]];
  
  if ([ret rangeOfString:@"(null)"].location != NSNotFound) {
    ret = self;
  }
	return ret;
}
